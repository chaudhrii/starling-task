...
This technical solution was not sufficient to impress Starling engineers. Main points are outlined.

- They are looking for a very particular 'perfect' solution.
- Starling **do not** read the README.md file
  - They do not compile or run the project
  - They share links within the company to gather opinions
- Starling prefer code commenting to logging
  - Engineers paid particular interest to logging potentially 'sensitive' data
    - Specifically the 'goal-name' should not be logged
- Round-up algorithm in the should use the `BigDecimal`
  - Starling do not accept the scenario outlined in the specification for justification for using a narrower type
  - Go wide with BigDecimal, assume extreme width when choosing types and do not go narrow and work up
- Starling found difficulty appreciating the usefulness of the Smoke Test
  - They would prefer not to have it at all
  - They were not aware of the documentation in the README as a justification and use
- Starling engineers found difficulty in understanding Spring Boot
  - During the interview one engineer even questioned whether it was a scalable technology
- Starling engineers found difficulty in understanding the Lombok library
  - Engineers during the interview admitted they did not know what it did
  - Starling do not use Lombok
- They prefer engineers with a maths background and focus on algorithmic challenges
  - You must describe the answer to the connection (node) distance between Starling and Apple inc in a LinkedIn network
    - Determine if the number of connections (distance) were 4 or less
  - You must know the data structure you would save the object into
